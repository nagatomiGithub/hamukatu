...
public class Dao extends DriverAccessor {
    ...
    public List<Article> getArticleList(String keyword, boolean isTrend) {
        ...
        try {
            ...
            if (keyword != null && !keyword.isBlank()) {
                ...
                sql = "SELECT a.*, (a.fav_count + (SELECT COUNT(*) FROM comment c WHERE c.article_id = a.id) + (LENGTH(a.body) - LENGTH(REPLACE(a.body, '#', '')))) as score FROM article a ORDER BY score DESC, entry_datetime DESC";
            } else {
            ...
            while (rs.next()) {
                list.add(new Article(rs.getInt("id"), rs.getString("title"), rs.getString("body"), rs.getString("editor_id"), rs.getInt("fav_count"), rs.getInt("dislike_count"), rs.getTimestamp("entry_datetime"), rs.getString("image_name")));
            }
    ...
    public void insertArticle(Article article) {
        Connection connection = this.createConnection();
        try {
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO article (title, body, editor_id, image_name) VALUES (?, ?, ?, ?)");
            stmt.setString(1, article.getTitle()); stmt.setString(2, article.getBody()); stmt.setString(3, article.getEditorId()); stmt.setString(4, article.getImageName());
            stmt.executeUpdate();
        } catch (SQLException e) { e.printStackTrace(); } finally { this.closeConnection(connection); }
    }
